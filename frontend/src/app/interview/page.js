...
import {
  ...
  Mic, MicOff, Video, VideoOff, Volume2, VolumeX, AlertCircle, CheckCircle2
} from 'lucide-react';
import Button from '@/components/ui/Button';
import Badge from '@/components/ui/Badge';
import InterviewerAvatar from '@/components/interview/InterviewerAvatar';
import ConfidenceMeter from '@/components/interview/ConfidenceMeter';
import LiveCoach from '@/components/interview/LiveCoach';
...
function useSpeechRecognition() {
  ...
  const start = useCallback(() => { if (ref.current) { ref.current._listen = true; setIsListening(true); try { ref.current.start(); } catch {} } }, []);
  const stop = useCallback(() => { if (ref.current) { ref.current._listen = false; setIsListening(false); try { ref.current.stop(); } catch {} } }, []);
...
function countFillers(text) {
  ...
  return FILLERS.reduce((n, f) => n + (lower.split(new RegExp(`\\b${f}\\b`, 'g')).length - 1), 0);
}
...
export default function InterviewSessionPage() {
  ...
  const [liveMetrics, setLiveMetrics] = useState({ eyeContact: 60, wordsPerMinute: 0, fillerCount: 0, confidence: 50 });
  ...
  // Live metrics update (every 2s)
  useEffect(() => {
    if (isComplete) return;
    const iv = setInterval(() => {
      const wc = answer.split(/\s+/).filter(Boolean).length;
      const elapsed = (Date.now() - (speechStartRef.current || Date.now())) / 60000; // minutes
      const wpm = elapsed > 0.05 ? Math.round(wc / elapsed) : 0;
      const fillers = countFillers(answer);
      const eyeContact = cameraOn ? 55 + Math.random() * 20 : 30;
      const pacingScore = wpm >= 120 && wpm <= 160 ? 100 : Math.max(0, 100 - Math.abs(wpm - 140) * 1.5);
      const fillerPenalty = Math.max(0, 100 - fillers * 5);
      const postureScore = cameraOn ? 60 + Math.random() * 20 : 40;
      const confidence = Math.round(eyeContact * 0.35 + pacingScore * 0.30 + fillerPenalty * 0.20 + postureScore * 0.15);

      setLiveMetrics({ eyeContact: Math.round(eyeContact), wordsPerMinute: wpm, fillerCount: fillers, confidence });
      setConfidenceHistory(h => [...h, { ts: Date.now(), confidence }]);
    }, 2000);
    return () => clearInterval(iv);
  }, [answer, cameraOn, isComplete]);

  // Coach timer (every 10s)
  useEffect(() => {
    if (!coachEnabled || isComplete) return;
    coachTimerRef.current = setInterval(async () => {
      const wc = answer.split(/\s+/).filter(Boolean).length;
      if (wc < 20 || !currentQuestion) return;
      try {
        setAuthTokenGetter(getToken);
        const res = await interviewAPI.coach({
          question: currentQuestion.question,
          partialAnswer: answer.substring(0, 500),
          questionType: currentQuestion.type,
          fillerCount: countFillers(answer),
        });
        if (res.data?.hint) setCoachHint({ hint: res.data.hint, type: res.data.type });
      } catch {}
    }, 10000);
    return () => clearInterval(coachTimerRef.current);
  }, [answer, currentQuestion, coachEnabled, isComplete, getToken]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Get AI reaction first
      try {
        const reactRes = await interviewAPI.react({ question: currentQuestion.question, answer });
        if (reactRes.data?.reaction) {
          setReaction(reactRes.data.reaction);
          setShowReaction(true);
          setAvatarState('reacting');
          await tts.speak(reactRes.data.reaction);
          await new Promise(r => setTimeout(r, 800));
          setShowReaction(false);
          setReaction('');
        }
      } catch { /* reaction is optional */ }
    ...
    finally { setSubmitting(false); setAvatarState('idle'); }
  };
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <InterviewerAvatar companyType={config.companyType} avatarState={avatarState} isSpeaking={tts.speaking} reaction={showReaction ? reaction : ''} />
            </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        {isComplete ? (
          ...
          <div className="flex-1 flex overflow-hidden">
              ...
              <div className="flex-1 flex flex-col p-4 overflow-hidden">
                ...
                {coachEnabled && coachHint && (
                  <div className="shrink-0 mb-3">
                    <LiveCoach hint={coachHint.hint} type={coachHint.type} visible={true} />
                  </div>
                )}
            ...
            <ConfidenceMeter liveMetrics={liveMetrics} />
          </div>
        )}
      </div>
    </div>
